   ...
   public void SellItem(Item item)
    {
        for (int i = 0; i < item.itemAttributes.Count; i++)
        {
            if (item.itemAttributes[i].attributeName == "Cost")
            {
                currentMoney = currentMoney + item.itemAttributes[i].attributeValue;
                Debug.Log("got the moneys");
            }
        }
        UpdateMoney();
        Debug.Log("upd money finished");
    }
