...
public class Inventory : MonoBehaviour
{
    ...
    public int[] GetInvAsArray()
    {
        updateItemList();
        updateItemIndex();
        int [] invArray = new int[(ItemsInInventory.Count * 2)];
        int x = 0;
        if (ItemsInInventory.Count == 0)
        {
            Debug.Log("u suck bro");
        }
        for (int i = 0; i < ItemsInInventory.Count; i += 2 )
        {
            Item it = ItemsInInventory[x];
            invArray[i] = it.itemID;
            invArray[(i+1)] = it.itemValue;
            x++;
        }
        return invArray;
    }

    public void SetArrAsInv(int[] sda)
    {
        ItemsInInventory.Clear();

        for (int i = 0; i < sda.Length; i+=2)
        {
            Item it = itemDatabase.getItemByID(sda[i]);
            it.itemValue = sda[i + 1];
            ItemsInInventory.Add(it);
        }
